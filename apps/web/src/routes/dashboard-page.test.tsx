...
describe("DashboardPage", () => {
  ...
  it("renders the signed-out Twitch auth gate", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /sign in with twitch/i })
    ).toBeInTheDocument();
    expect(
      screen.getByText(/lock the command deck to real twitch channels/i)
    ).toBeInTheDocument();
  });
  ...
  it("shows pending invite state and validates match creation inline until a pair is active", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /create match draft/i })
    ).toBeInTheDocument();
    expect(screen.getByText(/24h invite window/i)).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", { name: /create match draft/i })
    );
    expect(
      await screen.findByText(
        /activate a broadcaster pair before creating a match/i
      )
    ).toBeInTheDocument();
  });
  ...
  it("surfaces the signed-in moderator assignment error", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /assign mod/i })
    ).toBeInTheDocument();
  ...
  it("renders the recent activity panel when audit items are present", async () => {
    ...
    fetchMock.mockResolvedValueOnce(jsonResponse({ items: audit }));

    renderDashboard();

    fireEvent.click(await screen.findByRole("button", { name: /expand/i }));
    expect(
      await screen.findByText(/operational event rail/i)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/created an invite for @pixelriot vs @novarune/i)
    ).toBeInTheDocument();
  });
  ...
  it("shows inline validation and focuses the first invalid field for match creation", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/broadcaster pair/i), {
      target: { value: "link_1" },
    });
    fireEvent.click(
      screen.getByRole("button", { name: /create match draft/i })
    ...
    const error = await screen.findByText(
      /enter a match title with at least 3 characters/i
    );
